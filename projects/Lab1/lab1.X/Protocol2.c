...
void PacketHandling(void){
    ...
    if (PacketSending == TRUE){                         // If we are already sending a packet
        ...
        if (ReadPacket(&Read_Packet) == SUCCESS){       // If we can successfully read from buffer
            ...
            unsigned char pong_array[PONG_SIZE];        // temporary array for ping response
            ...
            switch (ID){                                // switch based of of packet ID 
                ...
                case (ID_PING):
                    ...
                    for (i=0;i<PONG_SIZE;i++){
                        pong_array[i] = Read_Packet.payLoad[i];         // move values to temp array
                    }
        
                    unsigned int intval = Convert_to_int(pong_array);   // conversion to int swaps endianess
                    intval = (intval >> 1);                             // divide by two
                    Convert_to_char(intval, pong_array);                // convert to char also swaps endianness

                    for(i=0; i<PONG_SIZE; i++){
                        OutPacket.payLoad[i] = pong_array[i];           // put modified temp array into out packet
                        checksum = Protocol_CalcIterativeChecksum(OutPacket.payLoad[i], checksum);
                    }
...
/**
 * @Function Send_Debug(unsigned char *str)
 * @param unsigned char *str
 * @return NONE
 * @brief Sends a debug message using an input string
 * @note Uses put char directly
 * @author <Mathias Eriksen>*/
void Send_Debug(unsigned char *str){
    ...
    for (i=0; i<length-1; i++){
        PutChar(str[i]);
        checksum = Protocol_CalcIterativeChecksum(str[i], checksum);        
    }
